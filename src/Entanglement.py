...
pygame.mixer.init()
try:
    # Ensure this file exists or comment it out for testing
    entangle_music = resource_path("Entanglement.ogg")
...
class StaticOverlay(QWidget):
    def __init__(self, parent=None):
        ...
        # We generally don't want focus, but we do want to catch mouse clicks
    ...
    def mousePressEvent(self, event):
        # Consume the click so it doesn't go to the desktop
        event.accept()
    ...
    def paintEvent(self, event):
        ...
        if self.frames:
            ...
            # Draw seamlessly scrolling static
...
class ErrorPopup(QLabel):
    def __init__(self, image_path, close_callback, parent_overlay, red=False, split_depth=[0,None,None]):
        ...
        print("Loading image:", image_path, os.path.exists(image_path))
    ...
    def move_step(self):
        ...
        # Get popup rect in GLOBAL coordinates
        ...
        bounds = screen.availableGeometry()  # excludes taskbar
    ...
    def animate_in(self):
        ...
        self.anim.setDuration(50)  # fast, snappy
    ...
    def place_randomly(self):
        ...
        if parent.multi_screen:
            ...
            # Convert GLOBAL → PARENT coordinates
...
class SettingsWindow(QWidget):
    ...
    def load_settings(self):
        try:
            ...
            self.challenge_strength.setValue(int(data.get("challenge_strength", 0.35) * 100))
            self.challenge_rarity.setValue(int(data.get("challenge_rarity", 0.15) * 100))
            self.challenge_min.setValue(data.get("interval_min", 30))
            self.challenge_max.setValue(data.get("interval_max", 90))
        except Exception:
            pass
    ...
    def save_settings(self):
        data = {
            ...
            "challenge_strength": self.challenge_strength.value(),
            "challenge_rarity": self.challenge_rarity.value(),
            "interval_min": self.challenge_min.value(),
            "interval_max": self.challenge_max.value(),
        }
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Remove FramelessWindowHint if you want to debug easier, 
        # but for the game effect, keep it.
        self.setWindowFlags(
            Qt.FramelessWindowHint |
            Qt.WindowStaysOnTopHint |
            Qt.Window
        ...
        self.challenge_strength = 0.35   # how hard values drift
        self.challenge_rarity = 0.15     # how often chaos happens
        ...
        # Create a dummy folder check so it doesn't crash if folder missing
    ...
    def start_entanglement(self):
        ...
        # Ensure we are the active window to capture keys (F4)
        self.activateWindow()
        self.raise_()
    ...
    def end_entanglement(self):
        ...
        # Restore base settings after challenge
        ...
        self.entanglement_active = False
        # Close any remaining popups (force cleanup)
        for p in self.popups:
            p.close_callback = None
            p.close()
            p.blockSignals(True) # Prevent recursion
        self.popups.clear()
    ...
    # ==========================================
    # KEY FIX: Paint 1-alpha background
    # ==========================================
    def paintEvent(self, event):
        if not self.entanglement_active:
            return
        painter = QPainter(self)
        # Painting with alpha=1 makes the window "exist" to the OS mouse,
        # but remains invisible to the human eye.
        painter.fillRect(self.rect(), QColor(0, 0, 0, 1))
        painter.end()

    # ==========================================
    # KEY FIX: F4 Kill Switch
    # ==========================================
...
def setup_global_hotkey(main_window):
    def on_activate_f9():
        if main_window.challenge_mode:
            return  # Challenge Mode owns control
